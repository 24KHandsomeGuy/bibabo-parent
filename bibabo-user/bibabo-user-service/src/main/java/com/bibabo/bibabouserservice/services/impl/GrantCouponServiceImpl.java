...
public class GrantCouponServiceImpl implements GrantCouponService {
    ...
    @Override
    public ResponseDTO grantCoupon(GrantCouponRequestDTO dto) {

        log.info("接收到发劵请求：{}", dto);

        if (dto.getCouponId() == null || dto.getCustId() == null) {
            return ResponseDTO.builder().fail("请求参数不能存在空").build();
        }

        Optional<Coupon> couponOptional = couponRepository.findById(dto.getCouponId());
        if (!couponOptional.isPresent()) {
            return ResponseDTO.builder().fail("不存在couponId为:" + dto.getCouponId() + "的优惠劵").build();
        }
        if (couponOptional.get().getCouponStatus() != 1) {
            return ResponseDTO.builder().fail("优惠劵couponId:" + dto.getCouponId() + "已经失效").build();
        }
        CustomerCouponRecord recordDB = customerCouponRecordRepository.findByCouponIdAndCustId(dto.getCouponId(), dto.getCustId());
        if (recordDB != null) {
            return ResponseDTO.builder().fail("优惠劵couponId:" + dto.getCouponId() + ",顾客id:" + dto.getCustId() + "已经发放优惠劵").build();
        }
        ...
        if (rAtomicLong.get() <= 0) {
            rAtomicLong.set(100);
            return ResponseDTO.builder().fail("优惠劵couponId:" + dto.getCouponId() + "库存不足发劵失败").build();
        }
